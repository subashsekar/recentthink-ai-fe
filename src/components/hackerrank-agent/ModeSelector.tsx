...
import { useHackerRankModes } from '@/hooks/hackerrank/useHackerRankQueries';
import { useUpdateSessionModeMutation } from '@/hooks/hackerrank/useHackerRankMutations';
...
export function ModeSelector({ className }: { className?: string }) {
  ...
  const { data, isLoading, isError } = useHackerRankModes();
  const modes = (data ?? []) as ModeInfo[];

  const activeSessionId = useHackerRankChatStore((s) => s.activeSessionId);
  const sessionModeId = useHackerRankChatStore((s) => s.session?.mode_id);
  ...
  const setModes = useHackerRankChatStore((s) => s.setModes);
  const setDefaultModeId = useHackerRankChatStore((s) => s.setDefaultModeId);
  const setSelectedModeId = useHackerRankChatStore((s) => s.setSelectedModeId);
