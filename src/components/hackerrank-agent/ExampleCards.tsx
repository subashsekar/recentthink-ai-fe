...
import { useHackerRankExamples } from '@/hooks/hackerrank/useHackerRankQueries';
...
export function ExampleCards({ onSelectExample }: ExampleCardsProps) {
  ...
  const { data: examples = [], isLoading, isError, refetch } = useHackerRankExamples();
      ...
      <div className="relative">
        ...
        >
          ...
          {!isLoading && !isError && examples.length === 0 && (
            <p className="py-4 text-sm text-muted">No example challenges available.</p>
          )}
