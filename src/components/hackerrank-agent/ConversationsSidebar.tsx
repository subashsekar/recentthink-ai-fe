...
import { useHackerRankHistory, useAiModels } from '@/hooks/hackerrank/useHackerRankQueries';
import {
  useDeleteSessionMutation,
  usePatchSessionMutation,
} from '@/hooks/hackerrank/useHackerRankMutations';
import { normalizeAnalyzeResponse } from '@/utils/hackerrankSession';
import { hackerrankApi } from '@/services/api/hackerrank';
import type { HackerRankSessionSummary } from '@/types/hackerrank';
...
export function ConversationsSidebar() {
  ...
  const setActiveSessionId = useHackerRankChatStore((s) => s.setActiveSessionId);
  const hydrateFromSession = useHackerRankChatStore((s) => s.hydrateFromSession);
  ...
  const handleTogglePin = async (conversation: HackerRankSessionSummary) => {
    try {
        ...
        {isError && (
          <div className="flex flex-col items-center gap-3 px-4 py-10 text-center">
            ...
            <button
              type="button"
              onClick={() => refetch()}
              className="glass-chip rounded-xl px-3 py-1.5 text-xs font-medium text-foreground"
            >
              Try again
            </button>
          </div>
...
function ConversationItem({
  ...
  conversation: HackerRankSessionSummary;
  isActive: boolean;
  isRenaming: boolean;
  renameValue: string;
  ...
  return (
    <>
      <button
        type="button"
        onClick={onRename}
        className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-foreground transition-colors nav-item-hover"
      >
