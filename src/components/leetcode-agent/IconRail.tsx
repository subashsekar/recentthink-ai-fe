...
const navItems = [
  ...
  { href: ROUTES.HACKERRANK_AGENT, icon: Trophy, label: 'HackerRank Agent' },
  { href: ROUTES.COURSES, icon: GraduationCap, label: 'Course Generator' },
  { href: ROUTES.DSA_PATTERN, icon: Shapes, label: 'DSA Pattern Coach' },
  ...
  return (
    <aside className="hidden w-[72px] min-w-0 shrink-0 flex-col items-center rounded-[24px] glass-panel py-4 shadow-lg lg:flex">
      ...
      <div className="mt-auto flex flex-col items-center gap-1 border-t border-border pt-3">
        <button
          type="button"
          className="flex h-10 w-10 items-center justify-center rounded-xl text-muted transition-colors hover-surface hover:text-foreground"
