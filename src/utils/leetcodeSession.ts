...
function formatEvaluatorOutput(value: unknown): string {
  ...
  const listSections: Array<[string, unknown]> = [
    ...
    ['Interview Follow-ups', obj.interview_follow_ups ?? obj.follow_up_questions],
  ];
...
function formatRoleOutput(role: LeetCodeAgentRole, value: unknown): string {
...
function messagesToRoleContent(
  ...
  for (const message of messages) {
    ...
    const role = (
      AGENT_ROLES.includes(agentName as LeetCodeAgentRole)
        ? agentName
        : String(message.role ?? '').toLowerCase()
    ) as LeetCodeAgentRole;
...
export function normalizeFollowUpResponse(
  ...
  const session: LeetCodeSessionDetail = current.session
    ? {
        ...current.session,
